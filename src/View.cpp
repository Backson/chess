...
void View::draw(float x, float y, const Position &position, Tile selection, Tile cursor) {
	ALLEGRO_BITMAP *target = al_get_target_bitmap();
	updateBuffer(position, selection, cursor);
	al_set_target_bitmap(target);
	al_draw_bitmap(_buffer, x, y, 0);
}

void View::updateBuffer(const Position &position, Tile selection, Tile cursor) {
	al_set_target_bitmap(_buffer);
